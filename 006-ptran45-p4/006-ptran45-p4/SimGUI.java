...
class SimGUI {
	...
	public void makeBottomButtons() {
		...
		play.addActionListener(new ActionListener() {
			private void toggle() {
				...
				((JButton)buttonPanel.getComponent(3)).setText((playing ? "Stop" : "Play"));
			}
	...
	public void genGraph() {
		...
		if(alg.graphEdgeType() == EdgeType.DIRECTED) {
			...
			//For directed graph algorithms, you need (1) a un-directed graph class
	...
	public void resetAlg() {
		if(alg == null) alg = new TopologicalSort();
	...
	public JPanel getSidePanel(int width, int height) {
		JPanel newPanel = new JPanel() {
			...
			public void paintComponent(Graphics g) {
				...
				if(!(alg instanceof TopologicalSort)) return;
				
				TopologicalSort ts = (TopologicalSort) alg;
				...
				g2.drawString("Sorted Nodes (start)", 0, shiftAmount);
				int i = 2;
				
				if(ts.queue != null) {
					for(GraphNode node : ts.queue) {
						g2.drawString(""+node.getId(), 0, i*shiftAmount);
						if(++i > maxDisplay+1) break;
					}
					if(ts.queue.size() > maxDisplay) {
						g2.drawString("...", 0, i++*shiftAmount);
						g2.drawString("...", 0, i++*shiftAmount);
					}
				}
				g2.drawString("Sorted Nodes (end)", 0, i++*shiftAmount);
			}
		};
